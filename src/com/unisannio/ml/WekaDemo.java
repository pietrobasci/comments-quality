...
public class WekaDemo {
   ...
   */
  public void execute(boolean train) throws Exception {
    // run filter
    //m_Filter.setInputFormat(m_Training);
    //Instances filtered = Filter.useFilter(m_Training, m_Filter);
	
    Instances filtered = new Instances(m_Training);

    // train classifier on complete file for tree
    if (new File("ML_file/" + m_Classifier.getClass().getSimpleName() + ".model").exists() && !train){
    	m_Classifier = (Classifier) SerializationHelper.read("ML_file/" + m_Classifier.getClass().getSimpleName() + ".model");
    }
    else{
    	m_Classifier.buildClassifier(filtered);
        SerializationHelper.write("ML_file/" + m_Classifier.getClass().getSimpleName() + ".model", m_Classifier);
    }
  ...
  public double doPrediction(String TestPath) throws Exception {
	...
	//Previsione
	//Instances filtered = new Instances(m_Training);
	Instances filtered = new Instances(new BufferedReader( new FileReader(TestPath)));
    filtered.setClassIndex(filtered.numAttributes() - 1);
	//m_Evaluation = new Evaluation(filtered);
	double prevision = m_Classifier.classifyInstance(filtered.get(0));
